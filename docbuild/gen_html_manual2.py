...
def build(lang):
    ...
    <table>
      ...
      <tr><td>13</td><td>""" + T("Matching bracket", "Matching bracket") + """</td><td>Navigation &rarr; Goto matching bracket</td><td>""" + T("Jumps to the bracket/brace/parenthesis matching the one under the caret - see “Bracket Matching” below for the red highlighting this also relies on.", "Springt zur Cursor-Position passenden Klammer - siehe „Klammern abgleichen“ weiter unten für die rote Einfärbung, auf der dieser Befehl mit aufbaut.") + """</td></tr>
    ...
    <h2>""" + T("Bracket Matching", "Klammern abgleichen") + """</h2>
    <p>""" + T("Whenever the caret sits directly before or after a bracket, brace, or parenthesis, both it and its matching partner turn red - this happens automatically as you type or move the caret, independently of the Navigation &rarr; Goto matching bracket command above, and regardless of whichever syntax colouring is currently active.",
          "Steht der Cursor unmittelbar vor oder hinter einer Klammer, einer geschweiften Klammer oder einer runden Klammer, färben sich sie und ihr Gegenstück rot - das geschieht automatisch beim Tippen oder Bewegen des Cursors, unabhängig vom Befehl Navigation &rarr; Goto matching bracket oben und unabhängig von der gerade aktiven Syntaxfärbung.") + """</p>
    <div class="note"><b>""" + T("Tip", "Tipp") + """:</b> """ + T(
        "Prefs &rarr; Misc &rarr; “Highlight block between braces” (unchecked by default) additionally shades the text between a bracket pair with a subtle, readability-preserving background the moment you use Navigation &rarr; Goto matching bracket - handy for confirming at a glance that a long bracketed passage is self-consistent. This shading only appears for that command, never for the always-on red colouring described above, and it disappears immediately once the caret moves outside the bracket pair.",
        "Prefs &rarr; Misc &rarr; „Highlight block between braces“ (standardmäßig deaktiviert) hebt zusätzlich den Text zwischen einem Klammernpaar mit einem dezenten, die Lesbarkeit nicht beeinträchtigenden Hintergrund hervor, sobald Navigation &rarr; Goto matching bracket verwendet wird - praktisch, um auf einen Blick zu erkennen, ob eine längere Klammer-Passage in sich stimmig ist. Dieser Hintergrund erscheint nur bei diesem Befehl, nie bei der oben beschriebenen, immer aktiven roten Einfärbung, und verschwindet sofort wieder, sobald der Cursor das Klammernpaar verlässt.") + """</div>
    """)
    ...
    <div class="note"><b>""" + T("Note", "Hinweis") + """:</b> """ + T(
        "Build Project also saves any open project file with unsaved changes first - including a hand-edited Makefile, not just source files - since both the Makefile regeneration above and the compiler itself always read straight from disk. Whether this happens automatically or asks first is controlled by Prefs &rarr; Project &rarr; \u201cSave Project Files Automatically\u201d (see below).",
        "Build Project speichert außerdem zuerst jede geöffnete Projektdatei mit ungespeicherten Änderungen \u2013 auch eine von Hand bearbeitete Makefile, nicht nur Quelldateien \u2013 da sowohl die obige Makefile-Regenerierung als auch der Compiler selbst immer direkt von der Platte lesen. Ob das automatisch geschieht oder vorher nachgefragt wird, steuert Prefs &rarr; Project &rarr; \u201cSave Project Files Automatically\u201c (siehe unten).") + """</div>
    ...
    <h2>""" + T("Saving before a build", "Speichern vor einem Build") + """</h2>
    <p>""" + T("Prefs &rarr; Project &rarr; \u201cSave Project Files Automatically\u201d (directly below \u201cProjects root\u201d) controls what Build Project does when an open project file - a source file or a hand-edited Makefile alike - has unsaved changes: checked, it saves everything immediately with no prompt; unchecked (the default), it asks once, listing every affected file, with the option to save them, build anyway from whatever is already on disk, or cancel the build entirely.",
          "Prefs &rarr; Project &rarr; \u201cSave Project Files Automatically\u201c (direkt unter \u201cProjects root\u201c) legt fest, was Build Project tut, wenn eine geöffnete Projektdatei \u2013 eine Quelldatei ebenso wie eine von Hand bearbeitete Makefile \u2013 ungespeicherte Änderungen hat: aktiviert wird alles sofort ohne Nachfrage gespeichert; deaktiviert (Standard) wird einmalig nachgefragt, mit einer Liste aller betroffenen Dateien und der Wahl, sie zu speichern, trotzdem mit dem Stand von der Platte zu bauen, oder den Build ganz abzubrechen.") + """</p>
    <h2>""" + T("Detecting Externally Changed Files", "Extern geänderte Dateien erkennen") + """</h2>
    <p>""" + T("Whenever AmigaED regains focus - you switch back to it from another application - it checks every currently open file against what's actually on disk. If one or more have changed outside AmigaED (another editor, a build tool, version control, ...), a dialog lists every affected file as a pre-checked box, so you can reload some or all of them from disk in one go. A file that also has unsaved changes of your own is listed too, but pre-unchecked and flagged with a warning, since reloading it would silently discard those changes - you have to tick it deliberately.",
          "Sobald AmigaED wieder den Fokus erhält - du also von einer anderen Anwendung zurückwechselst - werden alle aktuell geöffneten Dateien mit ihrem tatsächlichen Stand auf der Festplatte verglichen. Wurde eine oder mehrere außerhalb von AmigaED verändert (ein anderer Editor, ein Build-Werkzeug, Versionskontrolle, ...), erscheint ein Dialog mit einer Liste aller betroffenen Dateien, jede als vorab angehakte Checkbox - so lassen sich einzelne oder alle auf einmal von der Platte neu laden. Eine Datei mit zusätzlich eigenen, noch ungespeicherten Änderungen wird ebenfalls aufgelistet, aber vorab nicht angehakt und mit einer Warnung versehen, da ein Neuladen diese Änderungen stillschweigend verwerfen würde - hier muss bewusst angehakt werden.") + """</p>
    ...
    <ol class="steps">
      ...
      <li>""" + T("Every SEE ALSO line in an entry's text is scanned for other documented function names, each turned into a clickable link - clicking one jumps straight to that function's own entry, exactly as if you had typed it into the Filter field yourself. When the same short name exists in more than one library (e.g. <span class=\"code\">CMD_WRITE</span> in audio.device, carddisk.device, and trackdisk.device), the link prefers the entry belonging to the SAME library as the one you clicked from, since that is what a SEE ALSO reference means by AmigaOS convention.",
             "Jede SEE ALSO-Zeile im Text eines Eintrags wird nach weiteren dokumentierten Funktionsnamen durchsucht - jeder gefundene Name wird zu einem klickbaren Link, der direkt zum Eintrag dieser Funktion springt, genau so, als hätte man ihn selbst ins Filterfeld eingetippt. Existiert derselbe Kurzname in mehreren Libraries (z. B. <span class=\"code\">CMD_WRITE</span> in audio.device, carddisk.device und trackdisk.device), bevorzugt der Link den Eintrag aus derselben Library wie der gerade angezeigte, da ein SEE ALSO-Verweis laut AmigaOS-Konvention genau das bedeutet.") + """</li>
    </ol>
    <div class="note"><b>""" + T("Note", "Hinweis") + """:</b> """ + T(
