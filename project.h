...
public:
    ...
    // 3=AmigaOS 3.x, 4=ReAction, 5=MUI, 6=Assembler; 7=MUI/8=GadTools/
    // 9=ReAction GUI Builder Project - rev.160, see MainWindow::
    // importGuiBuilderProject() - created via an external GUI-builder tool
    // (Prefs > Tools) rather than one of AmigaED's own static templates),
    // or -1 if unknown (a hand-assembled .aep, an Import existing
    // Project..., or one saved before this field existed). Some toolchain
    // specifics depend on this - e.g. an AmigaOS 1.3 project's vbcc
    // Makefile must always use '+kick13', regardless of whatever the
